...
extern "C" {
/* ALFRED - Modified headers for new libsynk/libbrti libraries */
#include <fm.h>
#include <tm.h>
#include <mcast.h>
#include <brti.h>
...
/* Fontas -- add bgp out of band msgs */
void SendBgpOOB(int t, union sockunion lsu,union sockunion rsu, 
		struct agent_index index, RouteMsg* msg, double delta)
{
  char  work[BGP_MAX_PACKET_SIZE+RTI_BGP_HEADER_SIZE];
  struct MsgS* pMsg = (struct MsgS*)&work;
  int*         pMsgType = (int*)&pMsg[1];
  RTIBgpMsg_t* pm = (RTIBgpMsg_t*)&pMsgType[1];
  char str1[INET_ADDRSTRLEN];
  char str2[INET_ADDRSTRLEN];

  if(0)cout<<"SendBgpOOB() type: "<<t<<"binary addr: "<<
	 index.addr<<"port: "<<index.port<<
	 "msg length: "<<msg->Size()<<
	 "src "<<Bgp::inet_sutop(&lsu,str1)<<
	 "dst "<<Bgp::inet_sutop(&rsu,str2)<<endl;
  
  pMsg->TimeStamp = Scheduler::instance().clock() + lookahead + delta;
  *pMsgType = 666; // Note OOB Data
  pm->type = (RTIBgpMsgType_t)t;
  pm->lsu   = lsu;
  pm->rsu   = rsu;
  pm->index = index;
  
  if(t!=RTI_BgpPacket) { 
    cout<<"SendBgpOOB() Bad OOB type"<<endl;
    return ;
  }

  pm->msglength = msg->Size();
  msg->Reset();
  msg->GetAll(msg->Size(),(char*)pm->Payload);
  RTI_UpdateAttributeValues(ObjInstance, pMsg,
			    MSGS_SIZE(sizeof(*pm) + sizeof(int) + pm->msglength),
			    0);
}
...
if(*pMsgType == 601601)  
  ...
  if(*pMsgType == 666)
   { // Got oob data
     RTIBgpMsg_t* pm;
     pm = (RTIBgpMsg_t*)&pMsgType[1];
     char str_l[INET_ADDRSTRLEN];
     char str_r[INET_ADDRSTRLEN];
     
     if(0) cout<<"Got oob type "<<pm->type<<" length "
	       <<pm->msglength<<" src: binary addr: "<<
	     pm->index.addr<<" src port: "<<pm->index.port<<
	     " src su: "<<Bgp::inet_sutop(&pm->lsu,str_l)<<
	     " dst su: "<<Bgp::inet_sutop(&pm->rsu,str_r)<<endl<<flush;
     
     //Check if the dst is local
     Bgp* dst;
     
     if(Bgp::Ip2BgpMap) { /*if there is a BGP Registry 
			    in this federete*/
       dst = Bgp::Ip2BgpMap->FindBySu(pm->rsu);
       
       if(dst) {
	 if(0)cout <<"ReflectAtrributes() dst node "<< 
		str_r<<"FOUND in this system!" <<endl;
	 ...
	 case RTI_BgpPacket:	 
	   ...
	   if(0) cout <<"Received Bgp Packet" <<endl;

	   // find msg list for this connection, which is ided by the peer->rindex
	   for(itr = dst->m_msgs.begin();itr != dst->m_msgs.end();++itr)
	     if(memcmp(&itr->first,&pm->index,sizeof(struct agent_index))==0) { 
	       found = true;
	       break;
	     }
	   if (!found)
	     {
	       if (0) printf("ReflectAttributes() No msg list to store msg dst(%s), src(%s)"
			     ,str_r,str_l);
	       break;
	     }	 
	   // Store msg 
	   if(0) printf("ReflectAttributes() msg length(%d) stored dst(%s),src(%s)",
			pm->msglength,str_r,str_l);
	   
	   itr->second.second.push_back(RouteMsg(pm->msglength,(char*)pm->Payload));       
	   break;
	   
	 case RTI_BgpInitMsgList:
	   
	   RMsgList_t MsgList ;
	   pair<int,RMsgList_t> AgentMsgList(0,RMsgList_t()) ;
	   if(0)cout <<"Initiate Msg List" <<endl;
	   dst->m_msgs.push_back(pair<struct agent_index,pair<int,RMsgList_t> > 
				 (pm->index,AgentMsgList));	 
	   break;
	   
	 }
       } else { 
	 if(0)cout <<"ReflectAtrributes() dst node " << 
	   str_r<<" is not in this system" <<endl;
       }
     } else { 
       cout <<"Warning: No Bgp Registry in this system"<<endl;
     }
...
if(0)printf("Host %s ReflectTime %f MSize %ld MTime %f\n",
 ...
 Uncompress((unsigned long*)p->bits(),
            ...
            (MsgSize - sizeof(struct MsgS) - sizeof(int)) / 4);
#else
...
int   argc = 1;
  ...
  rlinkPktC = 0;
	...
	instance_ = this;
        rtiinstance_ = this;
        unsigned long long RequestCount = 0; /* ALFRED - convert to ull */
	instance_ = this;
...
Tcl& tcl = Tcl::instance();
  ...
  if (argc == 5) {
      if (strcmp(argv[1], "iproute-connect") == 0) {
        ...
        if (pRoute == NULL) {
          printf("Sim::rtischeduler: No pRoute for %08x:%d! ", ipaddr, dport);
