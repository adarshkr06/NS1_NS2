...
extern "C" {
/* ALFRED - Modified header list for libsynk/libbrti */
#include <fm.h>
#include <tm.h>
#include <heap.h>
#include <bufpool.h>
...
RTIRouter** ppContext;
 ...
 if (FreeVec.size())
   { // entries exist in free vector
     ...
     buf = FreeVec.back();
     buf -= sizeof(int)/sizeof(char); // Go back one int to find zone size
     if(0)printf("Reusing msg %p\n", buf);
     FreeVec.pop_back(); // Remove it
     if (*((int *)buf) < MsgSize) {
       char *tbuf;
       if(0)printf("buf entry too small, reallocating memory from %d to %d\n", 
           *((int *)buf), MsgSize);fflush(stdout);
       if ((tbuf = (char *)realloc(buf, (MsgSize + 32))) == NULL) {
   ...
   {
     if ((buf = (char *)malloc(MsgSize + 32)) == NULL) {
 ...
 char *tmp = buf + sizeof(int)/sizeof(char); // Need to offset by int
...
//Tcl& tcl = Tcl::instance();
  ...
  if(0)for(i=0;i<argc;i++)printf("%d - %s ", i, argv[i]);
  if(0)printf("\n");
  if(0)fflush(stdout);
  if (argc == 3) 
    {
...
char          work[255];
  ...
  if (ObjClass == NULL)
    {
      printf("%s can't get objclasshandle for %s, ip %s\n", hn, work,pIPAddr);
      exit(0);
    }
