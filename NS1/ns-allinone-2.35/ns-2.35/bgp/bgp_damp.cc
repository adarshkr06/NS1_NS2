...
int
Bgp::bgp_reuse_index (int penalty)
{
  int i;
  int index;
...
void 
Bgp::bgp_reuse_list_add (struct bgp_damp_info *bdi)
{
  int index;
...
void
Bgp::bgp_reuse_list_delete (struct bgp_damp_info *bdi)
{
  if (bdi->next)
...
int 
Bgp::bgp_damp_decay (time_t tdiff, int penalty)
{
  int i;
	  ...
	  if (bdi->lastrecord == BGP_RECORD_UPDATE)
	    {
	      ...
	      bgp_aggregate_increment (bgp, &bdi->rn->p, bdi->binfo,
				       bdi->afi, bdi->safi);   
	      bgp_process (bgp, bdi->rn, bdi->afi, 
			   bdi->safi, NULL, NULL, NULL);
	    }
...
int
Bgp::bgp_damp_withdraw (struct bgp_info *binfo, struct bgp_node *rn,
  ...
  if (bdi == NULL)
    {
	 ...
	 1. allocate a damping structure.
      ...
      bdi = (struct bgp_damp_info *) XCALLOC (MTYPE_BGP_DAMP_INFO, 
...
int
Bgp::bgp_damp_update (struct bgp_info *binfo, struct bgp_node *rn, 
...
int 
Bgp::bgp_damp_scan (struct bgp_info *binfo, afi_t afi, safi_t safi)
{
  time_t t_now, t_diff;
  struct bgp_damp_info *bdi;
...
void
Bgp::bgp_damp_info_free (struct bgp_damp_info *bdi, int withdraw)
{
  ...
  if (bdi->lastrecord == BGP_RECORD_WITHDRAW && withdraw)
    {
      bgp_info_delete ((struct bgp_info **) &bdi->rn->info, binfo);
      bgp_info_free (binfo);
      route_unlock_node (bdi->rn);
    }
  XFREE (MTYPE_BGP_DAMP_INFO, bdi);
}

void
Bgp::bgp_damp_parameter_set (int hlife, int reuse, int sup, int maxsup)
{
  ...
  damp->decay_array = (double*) XMALLOC (MTYPE_BGP_DAMP_ARRAY,
  ...
  damp->reuse_list = (struct bgp_damp_info **) XCALLOC (MTYPE_BGP_DAMP_ARRAY, 
  ...
  damp->reuse_index = (int*) XMALLOC (MTYPE_BGP_DAMP_ARRAY, 
...
int
Bgp::bgp_damp_enable (struct bgp *bgp, afi_t afi, safi_t safi, int half,
...
void
Bgp::bgp_damp_config_clean (struct bgp_damp_config *damp)
{
  /* Free decay array */
...
int
Bgp::bgp_damp_disable (struct bgp *bgp, afi_t afi, safi_t safi)
{
  /* Cancel reuse thread. */
  if (damp->t_reuse )
...
char *
Bgp::bgp_get_reuse_time (int penalty, char *buf, size_t len)
{
  time_t reuse_time = 0;
  struct tm *tm = NULL;
...
void
Bgp::bgp_damp_info_vty (struct vty *vty, struct bgp_info *binfo)  
{
  struct bgp_damp_info *bdi;
  time_t t_now, t_diff;
  ...
  if (CHECK_FLAG (binfo->flags, BGP_INFO_DAMPED)
    ...
    vty_out (zlog_default,LOG_DEBUG, ", reuse in %s",
...
char *
Bgp::bgp_damp_reuse_time_vty (struct vty *vty, struct bgp_info *binfo)
{
  struct bgp_damp_info *bdi;
  time_t t_now, t_diff;
  char timebuf[BGP_UPTIME_LEN];
