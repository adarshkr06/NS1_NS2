...
/* ALFRED - Modified header list for libsynk/libbrti libraries */
#include "tclcl.h"
#include "packet.h"
#include "ip.h"
...
ipaddr_t   targmask;
  ...
  if(0)for(int i=0;i<argc;i++)printf("%d - %s ", i, argv[i]);
  if(0)printf("\n");
  ...
  // ALFRED move to src-based (see argc == 7)
  /*
  if (argc == 5)
    {
      if (strcmp(argv[1], "add-route-passthrough") == 0)
        {// add-route-passthrough destip mask rlinkip
          ipaddr_t destip;
          ipaddr_t mask;
          ipaddr_t rlinkip;
          struct in_addr addr;
          if (strchr(argv[2], '.') != NULL)
            { // Dotted notation
              ...
              destip = ntohl(addr.s_addr);
            } 
          else
            {
              destip = strtol(argv[2], NULL, 0);
            }
          ...
          if(0)printf("addroute-pt dstip %08x mask %08x rlinkip %08x\n",
                      destip, mask, rlinkip);
          AddPTRoute(destip, mask, rlinkip, 0, 0);
          return(TCL_OK);
        }
      if (strcmp(argv[1], "add-route") == 0)
        {
          ...
          pHead = (NsObject*)TclObject::lookup(argv[3]);
          targmask = atol(argv[4]);
          if(0)printf("Adding route %s %s %s\n", argv[2], argv[3], argv[4]);
          if (pHead == NULL)
            {
              printf("add-route cannot find object %s\n", argv[3]);
              exit(1);
            }
          AddRoute(ipaddr, pHead, targmask);
          return(TCL_OK);
        }
    }
  */
  // ALFRED add add-route source support
  if (argc == 7) {
    if (strcmp(argv[1], "add-route") == 0) {
      struct in_addr addr;
...
void RTIRouter::recv(Packet* p, Handler* h)
{
  ...
  if(0)printf("%s RTIRouter::recv at %-8.5f, srcip %08x, destip %08x, destport %d\n", name_, Now(), rti->ipsrc(), rti->ipdst(), rti->ipdstport());
...
Tcl&          tcl = Tcl::instance();
  ...
  if(0)printf("rrecv time %f ipnodeid %ld RTIKIT nodeid %ld srcip %08x dstip %08x\n",
              Now(), rti->RTINodeId(), RTIKIT_nodeid, rti->ipsrc(), rti->ipdst());
  if (rti->RTINodeId() == RTIKIT_nodeid)
    { // Ignore our own packets
      if(0)printf("ignoring own packet %ld\n", RTIKIT_nodeid);
      Packet::free(p);
      return;
    }
  ipaddr = rti->ipdst();
  ...
  if(0)printf("%f: received pkt from remote, srcip %08x srdport %d dstip %08x dstport %d\n", Now(), rti->ipsrc(), rti->ipsrcport(), ipaddr, dport);
    ...
    ipaddr_t  smask)
{
  if(0)printf("RTIRouter::AddRoute, rt %p rtsize %d rtlength %d\n",
         rTable_, rTableSize_, rTableLength_);
  if(0)fflush(stdout);
