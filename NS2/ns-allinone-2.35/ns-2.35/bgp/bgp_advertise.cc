...
Bgp::baa_new ()
{
  return (struct bgp_advertise_attr *)
    calloc(1, sizeof (struct bgp_advertise_attr));
}

void
Bgp::baa_free (struct bgp_advertise_attr *baa)
{
  free(baa);
}

void *
Bgp::baa_hash_alloc (struct bgp_advertise_attr *ref)
{
...
unsigned int
Bgp::baa_hash_key (struct bgp_advertise_attr *baa)
{
  return Bgp::attrhash_key_make (baa->attr);
}

int
Bgp::baa_hash_cmp (struct bgp_advertise_attr *baa1, struct bgp_advertise_attr *baa2)
{
...
void
Bgp::bgp_advertise_free (struct bgp_advertise *adv)
{
  XFREE (MTYPE_BGP_ADVERTISE, adv);
}

void
Bgp::bgp_advertise_add (struct bgp_advertise_attr *baa,
...
void
Bgp::bgp_advertise_delete (struct bgp_advertise_attr *baa,
		      struct bgp_advertise *adv)
{
  if (adv->next)
...
struct bgp_advertise_attr *
Bgp::bgp_advertise_intern (struct hash *hash, struct attr *attr)
{
  struct bgp_advertise_attr ref;
  struct bgp_advertise_attr *baa;

  ref.attr = bgp_attr_intern (attr);
  baa = (struct bgp_advertise_attr *) hash_get (hash, &ref, (void *(*) ())baa_hash_alloc);
  baa->refcnt++;
...
void
Bgp::bgp_advertise_unintern (struct hash *hash, struct bgp_advertise_attr *baa)
{
  if (baa->refcnt)
    baa->refcnt--;
...
void
Bgp::bgp_adj_out_free (struct bgp_adj_out *adj)
{
  XFREE (MTYPE_BGP_ADJ_OUT, adj);
}

int
Bgp::bgp_adj_out_lookup (struct peer *peer, struct prefix *p,
...
struct bgp_advertise *
Bgp::bgp_advertise_clean (struct peer *peer, struct bgp_adj_out *adj,
...
void
Bgp::bgp_adj_out_set (struct bgp_node *rn, struct peer *peer, struct prefix *p,
		      ...
		      struct bgp_info *binfo)
{
  ...
  if (! adj)
    {
      adj = (struct bgp_adj_out *) XCALLOC (MTYPE_BGP_ADJ_OUT, sizeof (struct bgp_adj_out));
...
void
Bgp::bgp_adj_out_unset (struct bgp_node *rn, struct peer *peer, struct prefix *p, 
		   afi_t afi, safi_t safi)
{
  struct bgp_adj_out *adj;
  struct bgp_advertise *adv;
...
void
Bgp::bgp_adj_out_remove (struct bgp_node *rn, struct bgp_adj_out *adj, 
...

void
Bgp::bgp_adj_in_set (struct bgp_node *rn, struct peer *peer, struct attr *attr)
{
  ...
  adj = (struct bgp_adj_in *)XCALLOC (MTYPE_BGP_ADJ_IN, sizeof (struct bgp_adj_in));
  adj->peer = peer;
  adj->attr = bgp_attr_intern (attr);
...
void
Bgp::bgp_adj_in_remove (struct bgp_node *rn, struct bgp_adj_in *bai)
{
...
void
Bgp::bgp_adj_in_unset (struct bgp_node *rn, struct peer *peer)
{
  struct bgp_adj_in *adj;
...

void
Bgp::bgp_sync_init (struct peer *peer)
{
  afi_t afi;
  safi_t safi;
  ...
  for (afi = AFI_IP; afi < AFI_MAX; afi++)
    for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
      {
	sync = (struct bgp_synchronize *) XCALLOC (MTYPE_TMP, sizeof (struct bgp_synchronize));
	FIFO_INIT (&sync->update);
	FIFO_INIT (&sync->withdraw);
	FIFO_INIT (&sync->withdraw_low);
	peer->sync[afi][safi] = sync;
	peer->hash[afi][safi] = hash_create ((unsigned int (*) ())baa_hash_key,(int (*) ())baa_hash_cmp);
      }
}

void
Bgp::bgp_sync_delete (struct peer *peer)
{
  afi_t afi;
  safi_t safi;
