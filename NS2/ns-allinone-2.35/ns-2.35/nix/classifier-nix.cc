...
NsObject* NixClassifier::find(Packet* p)
{
	...
	hdr_cmn *th = hdr_cmn::access(p);	// common header (size, etc)
	hdr_ip* iph = hdr_ip::access(p);
	int datalen = th->size() - tcph->hlen(); // # payload bytes
	
	fprintf(stdout, " [%d:%d.%d>%d.%d] (hlen:%d, dlen:%d, seq:%d, ack:%d, flags:0x%x , salen:%d, reason:0x%x)\n",
					th->uid(),
					iph->saddr(), iph->sport(),
					iph->daddr(), iph->dport(),
					tcph->hlen(),
	...
	if(0)printf("Classifier %s, Node %ld\n",
							name(), pN->Id());
